...
describe BlueApron::Strategies::FavoriteStrategy do
  ...
  describe "#next_move" do
    ...
    context "when there are several previous moves" do
      # We really should use some test move classes, but I'll just use RPS for now
      let(:most_common_move) { BlueApron::Moves::Scissors }
      let(:least_common_move) { BlueApron::Moves::Rock }
      before(:each) do
        [{ move: most_common_move, times: 4 }, { move: least_common_move, times: 2}].each do |move_group|
      ...
      it { expect(subject).to be_a_kind_of(BlueApron::Moves::Rock) }
    end
  end
end